---
author: Roberto Bisceglie
date: 2026-05-22
license: CC BY-SA 4.0
title: The Fiction Engine Handbook
version: 1.0.0
---

## 1. Introduction

The character was hanging from a stone ledge, three hundred feet above a dry riverbed. Her fingers were slipping. The tension at the table was real—not because of the math, but because we all knew exactly what was at stake. If she fell, the campaign’s central mystery fell with her.

I remember looking at the player. She wasn’t looking at me. She was looking at her character sheet, squinting at a list of circumstantial bonuses. She was trying to remember if "Weather-Worn" applied to ancient granite or only to wooden structures. I was looking at the rulebook, trying to find the specific page that defined the difference between a "Controlled Descent" and a "Terminal Fall."

For five minutes, the character stopped hanging from the ledge. She didn’t fall, and she didn’t climb. She simply vanished. In her place was a group of friends arguing about a paragraph of text. This is **attention fracture**. By the time we found the rule and rolled the die, the sweat had dried. The urgency was gone. We weren't in the mountains anymore; we were in a library, doing research.

I kept noticing this pattern across decades of real, messy play sessions. Sometimes, the rules felt like wind at our backs, propelling the fiction forward. Other times, the rules felt like a wall we kept hitting—a procedural jam that forced us to stop looking at the fiction so we could look at the system.

I started calling the mechanism that causes this the **Fiction Engine**.

This book is not an academic taxonomy or a manifesto. It is a practical diagnostic tool. It is a lens designed to help you understand what happens to your attention, your cognitive load, and your group’s behavior during actual play. When you sit down at the table, where does your focus go? When does the system help you see the world, and when does it get in your way?

Before we begin, I need to be intellectually honest about what this lens is blind to.

This framework is a singular instrument among many. It is completely indifferent to tactical fairness, mathematical balance, and historical simulation. It will not help you build a "fair" combat encounter or a realistic model of plate armor. If those are your goals, this is the wrong book. We are here to ask how procedure shapes attention, and whether that attention returns to a changed fiction.

To do that, we’ll move through the framework one piece at a time. We will start by defining the Fiction Engine through the sensory experience of the "Windshield" and the "Road," then look at how cognitive load shapes the pacing of a session. From there, we’ll walk through three case studies—the minimal engine of 24XX, the pressure-cooker depletion of Breathless, and the flexible authority of Freeform Universal. Finally, we’ll look at the hard limits of this lens before concluding with **Thresholds**, a reference architecture designed to demonstrate these principles in action.

This is the lens I use. You’ll decide if it fits.

## 2. The Engine Under the Table

Imagine you are driving down a highway at night. When everything is going smoothly, you don’t actually look at your windshield. It is completely transparent. You look through it, focusing entirely on the **Road**: the shared situation: what is happening, what is at stake, and what is about to change.

The **Road** is not a story in the literary sense. It is not a pre-authored plot or a dramatic arc. It is the living diegetic situation: the characters, conditions, and events that all participants hold as real inside the game at this moment. When we talk about the fiction, this is what we mean.

The Road includes more than what every participant currently knows. Hidden motives, undiscovered rooms, secret dangers, and private knowledge are also part of the fiction when they are diegetic facts. They become visible to the table when play brings them into contact with the characters.

But the moment a heavy downpour hits, or a large insect splashes right in your line of sight, your focus instantly shifts. You are no longer looking at the road. You are staring directly at the **Windshield**: the rules, the stats, and the procedures. You’re watching the wipers struggle to clear the view.

This is exactly what happens every week at our gaming tables. Every RPG has some mechanism connecting the Windshield to the Road—some way of deciding when the rules step in and how they hand control back to the fiction. A **Fiction Engine** is the specific design paradigm in which that mechanism stays thin, transparent, and fiction-facing: it intervenes only when the situation is uncertain, and it returns a changed fictional state as quickly as possible.

Let’s look at how this plays out with two different groups trying to achieve the exact same thing: breaking down a locked wooden door to escape a guard patrol.

### Case A: The Frictionless Shift

At Marco’s table, the group is playing a game with a very direct Fiction Engine.

- **The Road**: Marco says, "I take a short crowbar from my pack and wedge it into the frame." He is looking at the imaginary door.
- **The Windshield**: The system doesn't ask Marco to hunt for a "Strength" stat or check a sub-rule for crowbars. It simply asks: Is the situation changing? Yes. Marco rolls a single die based on his character's general background. His eyes flicker to the die for two seconds.
- **Back to the Road**: The engine immediately translates that number into trouble. The door splinters open with a loud CRACK, but the crowbar snaps in half. Marco is already thinking about where to hide the broken tool.

The windshield remained transparent. The rules stepped in for a heartbeat to provide a consequence, and the group’s focus never really left the scene.

### Case B: The Mechanical Intermission

Now let’s look at Sara’s table. They are facing the exact same locked door, but their system is built differently.

- **The Road**: Sara says, "I try to force the door open with my shoulder."
- **The Windshield**: Suddenly, everyone stops looking at the door. The GM says, "Okay, that’s a Feat of Strength. Since it’s reinforced oak, the Difficulty Class is 18. Do you have any gear or help?"
- **The Flicker**: Sara’s focus is now entirely on the Windshield. She is squinting at her character sheet, scanning her skill list. Another player is flipping through the manual to see if the "Help" action costs a resource. For three minutes, the guard patrol doesn't exist. There is only the manual and the math.
- **Back to the Road**: Eventually, the die is rolled and the procedure ends. The group has to "re-load" the fiction: "Wait, where were the guards again?"

The group spent three minutes staring at the glass, adjusting the wipers. Sara's system required a heavy procedural detour before it allowed anyone to find out if the door actually opened. Notice that for those three minutes the door was neither open nor shut. The fiction wasn't slowed—it was **suspended**. The situation couldn't change until the procedure finished.

### Defining the Lens

To understand why these two moments feel different, we need to name what is happening to our attention. We’ll use three terms, defined through the eyes of the player:

1.  **Fictional State**: This is something you can see and respond to on the Road. If your crowbar snaps (Case A), that is a fictional state. You can talk about it, you can feel the loss of the tool. If you have a "+2 Difficulty Modifier" (Case B), that is not a fictional state. You can’t "see" a modifier in the world. It’s just paint on the windshield.
2.  **Overhead**: Procedural steps that produce neither fictional change nor the conditions for it. When Sara spends three minutes scanning her skill list, that’s overhead—it buys nothing in fictional terms. But not every non-diegetic element is overhead. Safety agreements, campaign records, and pacing clocks also exist outside the fiction, yet they support the coherence that makes fictional change possible. This framework targets *dead* overhead specifically: procedural weight that purchases no fictional return.
3.  **The Transparency Threshold**: This is the tipping point where a rule stops being a lens you look *through* and starts becoming an obstacle you look *at*. A rule can briefly enter your field of vision (you glance at the die, you name a consequence) and that is not a problem. The problem is a Windshield that holds your attention without handing you back a changed situation. Below the threshold, a procedure steps in, does its work, and returns you to the Road. Above it, the procedure becomes a thing in itself: the center of the table’s attention, disconnected from whatever was happening in the fiction. In Case A, the engine stayed below the threshold. In Case B, it crossed it, forcing you to stop being a character so you could be a book-keeper.

Neither of these designs is inherently "better." If your group loves the tactical puzzle of optimizing bonuses, Case B is satisfying. But this framework is designed to look at a very specific trade-off: **cognitive load**.

The shift from "What would my character do?" to "What does the manual say?" costs nothing if the rules hand you back a changed situation immediately. It costs something real when they don't. By understanding where your favorite game sits on this spectrum, you can begin to answer the question:

**What is this rule doing to your attention right now?**

A note on vocabulary: throughout this book, **Road** and **Windshield** are table-facing terms for what the framework calls **diegetic fiction** and **procedural layer**. **Attention fracture** names the subjective experience of crossing the **transparency threshold**. **Cognitive load** is what the framework measures as the weight of **procedural overhead**. The handbook uses the experiential vocabulary; the framework uses the structural vocabulary. They refer to the same phenomena.

## 3. Attention, Pacing, and the Group

Where does your attention go during a game?

If you could map your focus during a typical session, you would see it flickering between different layers. You are tracking what is happening on the **Road**: the guard’s hand moving toward his sword, the smell of damp stone, the flickering torchlight. Simultaneously, you are decoding the **Windshield**: checking if you have the right skill, calculating a bonus, or remembering if a specific resource is spent. 

This brings us to **cognitive load**. In this book, cognitive load isn't a theory; it is a physical feeling. It is the friction you feel when a rulebook stops being a tool and becomes an unwanted "third participant" in the conversation. You’ve felt this: the moment the flow of a scene dies because you have to stop and argue with the manual. In that moment, the characters stop existing, and your attention is pulled entirely away from the ledge and onto the paper.

A Fiction Engine keeps the route from fictional intent to fictional change clear; cognitive load is what we feel when that route clogs.

### Attentional Distribution

How we handle this load depends on who is at the table.

In a **traditional group**, the Game Master often carries the heaviest load. They are the ones tracking the world, the NPCs, and the majority of the rules. Players focus on their individual characters. If the engine is heavy, the GM becomes a processor, constantly translating math into fiction while the players wait. If the engine is light, the GM can stay on the Road with the players.

In **solo play**, the situation is even more stark. A solo player is the GM, the player, and the rule-checker all at once. For them, every ounce of "overhead" is a direct drain on their creative energy. If a solo engine is too thick, the "game" part quickly suffocates attention to the fiction.

### Engine Profiles: Pacing and Momentum

We can categorize engines by how they affect the **momentum** of the group.

#### The Stalling Engine
Imagine a scene where a character is trying to bypass a security terminal. The player describes the attempt. The GM asks for a hack check. The player rolls, then realizes they need to check their equipment for a specific deck type. The GM looks up the security level. Another player remembers a rule that might contradict this. Five minutes pass. By the time the result is determined, the tension is gone. The engine has stalled the fiction.

**Is stalling a "sin"?** No. If your group explicitly wants slow, tactical calculation and deep mechanical mastery, a stalling engine is exactly what you want. The friction *is* the fun. But if you wanted a fast-paced heist, the stalling engine is your enemy.

#### The Accelerating Engine
Now imagine the same scene. The player describes the attempt. The GM notes that the situation is high-pressure and asks for a single roll. The result is a partial success. "The door slides open," the GM says immediately, "but the alarm starts chirping. You have ten seconds before the guards arrive. What do you do?" The resolution happened in seconds. The situation didn't pause; it transformed. The engine has accelerated the fiction.

**Is acceleration a cure-all?** Not necessarily. If an engine moves too fast, it can feel "thin." The group might feel like their mechanical choices don't matter, or that they are moving fiction without meaningful resistance.

The question isn't which profile is better. The question is: **Is the engine's pace matching the group's intent?**

### The Table's Orientation

There is a failure mode that has nothing to do with the rules themselves.

Even a correctly designed fiction engine, one built around thin mediation, fictional consequences, and no dead overhead, can still fail at the table. This happens when participants are not holding the same model of what the game is doing.

One player treats the session as a tactical problem: they want to optimize outcomes and manage consequences efficiently. Another treats it as character study: they want to inhabit the situation and let it change who they are. A third is there for the world, more interested in what the setting reveals than in any single character's arc. None of these orientations is wrong. But when they run simultaneously, without prior agreement, the fiction fragments. The GM delivers a consequence; one player treats it as an obstacle to route around; another carries it as a wound. They are in the same room, rolling the same dice, and playing in different fictions.

This is a social condition, not a mechanical one. Rules cannot solve it. What design can do is make the game's orientation legible from the start: what kind of object this is, what it asks of participants, what it will produce. What the table can do is state expectations before play begins and treat the fiction as a genuinely shared object rather than a stage for individual intent.

A fiction engine is a social agreement before it is a mechanical system. The rules formalize that agreement. They cannot substitute for it.

### Transparency Is Not Enough

One more thing must be said before we look at specific engines.

This framework measures the relationship between procedure and attention. It treats low overhead as desirable. But low overhead is a means, not an end. An engine that removes all friction while producing vague, weightless, or arbitrary consequences has not solved the problem. It has replaced one failure with another.

Attention fracture is the symptom. Dead overhead is the diagnosis. Fictional change is the test.

A fiction engine must stay transparent while still preserving **uncertainty** (the outcome is genuinely in doubt), **agency** (what the player chose affected what happened), and **consequence** (the result changes what is now true in a way that matters to the fiction going forward). These three are what make the Road worth looking at. Without them, clearing the Windshield is just clearing glass with nothing behind it.

Keep this in mind when evaluating the engines below. The question is not only "Is the procedure thin?" It is: "Does the procedure, thin as it is, still produce outcomes that are uncertain, chosen, and consequential?"

## 4. 24XX: The Minimal Engine

Imagine sitting down to run a game of 24XX for the first time.

The first thing you notice is the physical space. There is no thick manual on the table. You have a single page of text. Before the first die is cast, you feel a peculiar sense of openness. There is a promise in that thinness: the rules are going to stay out of the way. You can feel the **Road** more clearly because there is so little **Windshield** in front of it.

But as the session begins, you might also feel a flicker of **procedural vertigo**. There is no rule for cover, no table for weapon ranges, and no specific rule for how to handle a zero-gravity firefight. The engine is at its operational minimum.

### The Case Study: The Moon Base

You are a group of specialists infiltrating a corporate research station on the moon. You’ve reached the server room, but the door is protected by a biometric scanner. "I’ve got the lead researcher’s severed hand in my pack," one player says. "I’m going to try to trick the scanner into thinking he’s the one standing here."

The GM doesn't look up a "Security Bypass" skill. They look at the player and ask: "Is this something your character’s background covers?" The player is a Bio-Engineer. The GM nods. "Roll your d10."

The die hits the table: a **3**.

In 24XX, a 3 is a "setback." The GM doesn't consult a table of consequences. They look back at the fiction. "The scanner accepts the hand," the GM says, "and the door hisses open. But as you pull the hand away, the cold storage seal on your pack breaks. The scent of preservative chemicals is flooding the hallway. Anyone nearby is going to know something is wrong. What do you do?"

### The Design Commitment: Maximum Continuity

The commitment of a minimal engine like 24XX is **minimum overhead.** Because the rules are so sparse, the engine almost never crosses the transparency threshold. You never have to stop the scene to look up a rule because there are no rules to look up beyond the core resolution. 

Consider the "Defense" mechanic: *Say how one of your items breaks to turn a hit into a brief hindrance.* 

This is a surgical piece of engineering. There are no "Armor Class" numbers or "Damage Reduction" calculations. If your character is hit, you don't look at the manual; you look at your character. You look at your **Road** state. "The bullet hits my medkit," you say. The engine has produced a new **Fictional State** (a broken medkit) that everyone can see and respond to, while keeping the **Windshield** completely clear. The transition from "I do this" to "This happens" is nearly instantaneous. 

### The Cost of Thinness (Intellectual Honesty)

However, this thinness comes with a heavy price tag: **relative to more scaffolded designs, the engine offers no assistance.**

In a more procedural game, a low roll might trigger a specific mechanical penalty that the rules handle for you (e.g., "Take 4 damage"). In 24XX, a low roll only produces the word "setback." It is entirely up to the human beings at the table to decide what that means.

If the fiction runs thin, the engine offers nothing. This can happen when the group is tired, the situation is vague, or no one can think of an interesting consequence. There is no table to roll on, no "GM Move" list to consult. When the group’s creative output flags, the engine doesn't break; it simply offers **total silence**.

**Behavioral Portrait**: This engine is for groups who want the rules to act as a silent referee that only speaks when asked to resolve uncertainty. But if you expect the system to provide creative friction or "correct" answers, the absence of scaffolding can feel like falling into an empty room.

## 5. Breathless: The Engine Under Pressure

You are cornered. The air in the ventilation duct is hot and tastes like ozone. You can hear the scrap of metal on metal. The hunter is close. You reach for your sidearm.

Early in the mission, your "Aim" was a d10. You were steady, focused, and fast. Now, after three hours of running and two near-misses, that d10 has degraded. You look at your character sheet. Your "Aim" is now a d4.

The hunter rounds the corner. You have one shot. You aren't just looking at a number; you are looking at your character’s trembling hands. You are looking at the sweat stinging your eyes. That d4 is the feeling of having almost nothing left.

### Concept: Depletion as Fictional Information

In many games, a penalty is just a mathematical hurdle. If you are "Exhausted," you subtract 2 from your roll. You do the math, you roll the die, and you move on. The penalty is paint on the **Windshield**, often disconnected from the **Road**.

The Breathless engine handles this differently. Here, **resource depletion is fictional information.**

When your die degrades from a d10 to a d4, the engine is telling everyone at the table something specific about the situation. The number on the sheet is a visible condition, not a mathematical penalty.

Consider a character trying to leap across a gap:

- **Early Session (d10)**: The GM describes the jump as a feat of athleticism. The character is fresh, their boots have grip.
- **Late Session (d4)**: The GM frames the exact same jump differently. "You’re gasping for air," the GM says. "Your legs feel like lead. You aren't sure if you have the strength to make the clearing."

The d4 has changed the fiction. It has produced a new **Fictional State**, the character’s physical limit, that everyone at the table can see, respond to, and reason about. The number on the sheet and the situation in the world are the same thing.

### The Point of Friction: Drama vs. Administration

The Breathless engine works best when the degradation forces a character choice. But it can easily tip into **Administrative Overhead**.

- **Fictional Work**: A player looks at their d4 and says, "I can't risk another roll. I'm going to try to talk our way out of this instead." The engine has successfully driven a dramatic character decision.
- **Active Help (Momentum)**: When a player decides to "Catch their Breath" to reset their dice, the GM is instructed to *look at the scene and introduce a new complication for the group.* This is a vital piece of the engine. It doesn't just let the players "pause" to heal; it forces the **Road** to transform. The engine actively helps the GM keep the pressure high, ensuring that every "breather" is actually a pivot into a new fictional crisis.
- **Administrative Overhead**: A player spends sixty seconds debating whether they should use the "Catch Your Breath" mechanic purely to optimize their math for a future roll, regardless of whether the character would actually stop to rest. The group is no longer looking at the Road; they are staring at the Windshield, arguing about the wipers.

**An Honest Note**: The shrinking die is not fictional by itself. It becomes fiction-engine material only when it is expressed as an observable condition: shaking hands, ragged breath, tunnel vision, frayed nerves, depleted ammunition—some state the characters can perceive and respond to. If the table treats degradation as a math problem to optimize, the engine collapses into bookkeeping. The number has to become the trembling hands, or it stays paint on the Windshield.

## 6. Freeform Universal: The Flexible Engine

The promise of a flexible engine is simple and seductive: a tool that shapes itself to the fiction rather than imposing a shape upon it.

When you look at a game like Freeform Universal (FU), you feel a sense of liberation. There are no lists of specific skills, no rigid combat rounds, and no setting-specific constraints. The engine promises to be a "universal solvent" for fictional uncertainty. Before play begins, the appeal is the total absence of walls. 

But flexibility is not a free gift. It is a trade-off.

### Mechanics at the Table: Fictional Authority

FU functions by having the table propose **Closed Questions**: *"Do I leap the chasm?"* or *"Do I convince the guard?"*. The engine then resolves these using a resolution grammar: **Yes/No** modified by **And/But**.

Imagine the scene in the flooded subterranean temple. You are trying to keep your balance on a slick stone altar while fending off a swarm of leeches.

- **The Closed Question**: "Do I clear the leeches from the altar?"
- **The Windshield Negotiation**: To resolve this, you and the GM must negotiate the physics of the scene. You point out that your character is *Nimble* (+1 bonus die). The GM counters that the altar is *Slick* (-1 penalty die). You then mention your *Heavy Blade* (+1 bonus die). 
- **The Math**: Using the **Cancelling Out** rule, the bonus and penalty dice negate each other, leaving you with one bonus die. You roll 2d6.
- **The Result**: You roll a **5** and a **2**. In FU, a 5 is "No, but..." and a 2 is "Yes, but...". Since you are rolling a bonus die, you get to choose the result you prefer. You choose the 2.
- **The Return to the Road**: "I clear the leeches," you narrate, "but as I swing the blade, the momentum tilts the altar further. I’m sprawling toward the edge." The GM adds a **Condition** to your sheet: *Vulnerable*.

This is a deep design commitment to **fictional continuation**. It is impossible for the resolution to end in a stalemate. The engine’s only job is to ensure the question "What happens next?" always has a transformed answer.

### The Cost of Flexibility: Total Creative Weight

The weight of this engine is felt in the **creative friction** required by the group. Because there are no pre-set skills, you are effectively building the "physics" of the world in every scene. In the example above, the GM and the player had to decide *right now* if being *Nimble* was more important than the altar being *Slick*. 

Compare this to the other case studies:

- In **24XX**, the engine is minimal and gets out of the way. It gives you a moon base and a d10, then stays silent.
- In **Breathless**, the engine provides pressure. It tells you exactly how tired you are by shrinking your dice.
- In **FU**, the rules provide a grammar for transformation, but they offer **no content of their own**.

Consider the "Yes, but..." result above. The engine *forced* a transformation, but it didn't *propose* one. It didn't tell the GM about the tilt of the altar or the *Vulnerable* condition. If the GM is tired, or the group’s creative energy runs thin, a result like "No, but..." can feel like a burden. You have to invent the "but" on the spot. If the table goes quiet, the engine offers no structural bridge. 

You have total freedom, but you carry the total weight of the world. Every "And" and "But" is a creative tax paid by the humans at the table to keep the Road moving.

### A Map of the Case Studies

We have looked at three different engines. We can compare them by asking one question: **"How much help does the group need from the manual to keep the fiction moving?"**

1.  **24XX**: Minimal help. The engine is a silent referee. It gives you a setting and then gets out of the way. It requires a generative group that can fill the silence when the rules offer none.
2.  **Breathless**: Active help. The engine is a pressure cooker. It provides a constant, mechanical sense of urgency that forces the fiction to pivot toward desperation, even when the group's creative energy flags.
3.  **Freeform Universal**: Grammatical help. The engine is a universal solvent. It provides the grammar for how situations transform, but demands that the group supply 100% of the creative friction to fuel those transformations.

None of these are better or worse. They are just different ways of answering the question of how much "scaffolding" you want on your Windshield.

## 7. The Limits of the Lens

To trust a tool, you must know what it cannot measure. Throughout this book, we have prioritized thin mediation, low cognitive load, and a clear view of the Road. But it is time to be the ultimate skeptic of this framework.

Where does the vocabulary of the "Fiction Engine" completely run dry?

### Authority Drift

The fiction engine relies on some mechanism for distributing narrative authority: a referee who mediates between the fiction and the players, a shared authorship structure, an oracle in solo or low-referee play. Any of these can drift.

A referee ruling that should produce a fictional consequence instead generates procedural negotiation. An oracle output that should read as a fictional event instead reads as a task: "introduce a complication," "add tension," "describe something hidden." The players receive an administrative prompt rather than a fact about the world. They must do significant interpretive work before the fiction can advance.

When this happens, the mechanism has become a bottleneck rather than a conduit. The signal is familiar: more time spent on the Windshield than the Road. But the cause is different. The rules may be functioning correctly. The authority mechanism is the thing generating overhead where it should be generating fiction.

In solo play this is particularly easy to miss. An oracle that consistently produces prompts instead of events can feel like the game is working, because you are always generating answers. But if those answers require invention before they become fictional facts, the oracle has crossed into dead overhead, and the engine is stalling at precisely the point it was supposed to sustain the fiction.

The fix is the same in each case: the authority mechanism must produce fictional events, not procedural tasks. "The guard is suspicious" is a fictional fact. "Generate suspicion" is overhead wearing the disguise of an answer.

### Paradigm Boundaries: When Overhead is the Point

There are vast territories of play where the concept of "overhead" as a "tax" is fundamentally incorrect. In these paradigms, the "thick windshield" is not a bug; it is the intended experience.

#### 1. Heavy Historical Simulation
If you are playing a game designed to accurately model the ballistics of 19th-century rifles or the logistics of a Roman legion, you *want* to look at the rules. The pleasure of the game comes from the rigor of the procedure. The math is not "getting in the way" of the world; the math *is* the world. In this context, the Fiction Engine lens is useless because it sees complexity as friction, whereas the players see it as fidelity.

#### 2. Competitive and Tactical Wargaming
In games built around tactical grids, hidden information, and strategic outplaying, the rules are the terrain. Players *must* stare at the Windshield to win. They are looking for mechanical advantages, optimizing action economies, and calculating probabilities. To "measure the attention fracture" in a tactical skirmish game is to misunderstand the game's goal: the players aren't trying to feel the "wind at their backs"; they are trying to solve a mechanical puzzle.

#### 3. Ritual and Complexity
Sometimes, the sheer weight of a procedure provides a sense of ritual. A complex downtime phase or a detailed character creation process can be deeply satisfying precisely because it requires you to step out of the fiction and engage with the machine. 

### Respecting the Edge

If you apply the Fiction Engine lens to a dense, tactical RPG, you will see massive amounts of "overhead" and constant "transparency thresholds" being crossed. But that doesn't make it a bad game. It just means you have moved into a different design paradigm.

This framework is a singular instrument. It measures the connection between human attention and fictional momentum. It is not equipped to analyze tactical balance, mathematical fairness, or simulation accuracy.

If your goal is to lose yourself in the gritty realism of a logistics spreadsheet or the competitive thrill of a tactical map, put this book down. You are already exactly where you want to be. But if you find yourself frustrated that the fiction keeps getting suspended so the "game" can happen, then the edge of this lens is exactly where your work begins.

We have diagnosed the friction. Now, let’s look at a system built specifically to remove it. 

Turn the page to **Thresholds**. Let's return to the Road.

## 8. Thresholds: A Reference Architecture

Throughout this book, we have used the Fiction Engine lens to analyze existing designs. We have diagnosed overhead, mapped the transparency threshold, and felt the friction of the "third participant." But diagnosis is only the first step. To truly understand the framework, we need to see what it looks like when a system is built surgically to solve these specific problems.

This final chapter introduces **Thresholds**.

*Thresholds* is not presented here as a "universal" or even a "complete" RPG in the traditional sense. It is a **reference architecture**. It is a system built specifically to demonstrate every principle of the Fiction Engine framework in a single, playable document.

### The Design Commitment: Infrastructure for the Road

In *Thresholds*, the Windshield has been made as thin as possible. Every mechanic is held against one test: *Does this produce a new fictional state, or is it just dead overhead?*

Consider how these rules shape behavior at the table:

- **Descriptive Harm**: By removing hit points and damage numbers, *Thresholds* forces players to look at the Road. When your character is "bleeding from the ribs," you don't calculate how many hits you have left. You change your behavior. You seek cover. You bargain. You feel the **caution** that comes from inhabiting a fragile human being, rather than the confidence that comes from tracking a numerical buffer.
- **Fictional Facts as Stats**: There are no +1 modifiers. Instead, your character is defined by **Truths**. These truths don't change the math; they change what is *possible*. A character who "knows the songs that calm the dead" doesn't get a bonus to a roll; they are simply the only person in the room who can attempt to talk to the ghost. This keeps the group’s focus entirely on the fictional situation.
- **Enforced Continuation**: The resolution table eliminates the "nothing happens" result. Every roll transforms the situation, handing control back to the Road immediately.

### The Logic of the Treatment

Before you read the rules, it is worth seeing how the diagnosis we’ve built throughout this book is applied to the artifact. *Thresholds* is built to match the framework in the following ways:

-   **Every rule points back to a Fictional State.** Traits, harm, equipment, magic, and time all matter because they change what is true in the world. There are no +1s or generic modifiers.
-   **Resolution is thin.** One 2d6 roll converts fictional intent into fictional change. The "Windshield" is a single piece of glass.
-   **Outcomes enforce momentum.** The resolution table eliminates the "nothing happens" result. Every roll creates a new situation rather than a dead end.
-   **Zero abstract pressure economies.** There are no hit points, stress points, or generic supply pools. If you are in trouble, it’s because of the situation on the Road.
-   **Authority is explicit.** The referee maintains coherence; players state intent and method; the oracle (if used) produces only concrete fictional facts.
-   **Growth is fictional, not numerical.** Characters change because events change them, not because they’ve accumulated enough points to buy a +1.

### A Diagnostic Checklist

Before reading the rules that follow, or when evaluating any procedure in any game you play or design, hold it against these six questions:

1. **Fictional state.** Does this rule produce a change to the shared situation, or only a change to the game record?
2. **Overhead.** Is any procedural weight it generates dead overhead, or does it support the conditions that make fictional play possible?
3. **Suspended outcomes.** Does the resolution produce a playable result immediately, or does it defer the fiction pending further procedure?
4. **Authority drift.** Does the authority mechanism (referee, oracle, or shared authorship) deliver fictional facts or administrative prompts?
5. **Table orientation.** Are all participants treating the fiction as the shared object of play before the first die falls?
6. **Legibility.** After the result, does everyone understand what is now true, what can be seen or responded to, and what choices are available next?

A rule that fails question one is a candidate for removal. Rules that fail questions two through six may not be broken, but they carry a risk the table should name before it becomes a habit.

### The Test Drive

If the previous chapters were the diagnosis, *Thresholds* is the treatment. It is a tool for practice: a way to experience what happens to your group's attention when the procedural mediation is minimized.

The full, verbatim rules follow in the Appendix. Use them as a reference, a starting point for your own experiments, or simply as a final demonstration of the lens in motion.

One cost should be named honestly. *Thresholds* buys procedural transparency by moving interpretive labor onto the referee and table. Its rules are thin, but the table must be able to produce concrete consequences from fictional context. When the context is vague or the group's creative energy runs low, the engine can still stall—the same cost identified for 24XX and FU. Thin rules require a generative table.

Now, return to the Road.

## License

The Fiction Engine Handbook © 2026 by Roberto Bisceglie is licensed under Creative Commons Attribution-ShareAlike 4.0 International. To view a copy of this license, visit https://creativecommons.org/licenses/by-sa/4.0/

## Appendix: Thresholds - A fiction-engine tabletop RPG

### 1. What this game is

**Thresholds** is a rules-light tabletop RPG about characters entering unstable situations and changing them through play.

The rules do not try to model the world exhaustively. They answer one question:

> **When a character tries something uncertain and consequential, what happens in the fiction next?**

There are no hit points, classes, levels, action economies, tactical grids, fate points, or victory conditions. Characters are defined by fictional facts. Harm, advantage, danger, equipment, exhaustion, fear, reputation, trust, and opportunity all matter only insofar as they exist inside the fiction and change what can plausibly happen.

The referee’s role is to keep the world coherent, responsive, and surprising. The players’ role is to say what their characters attempt from inside the situation.

Before play begins, the table should agree on its orientation:

> We are here to treat the fiction as the shared object of play. We will make choices from inside the situation. We will let consequences change the world and the characters. We will not treat the rules as a tactical puzzle or a way to win.

Before play, briefly name any material that should be excluded, softened, or handled off-screen. This agreement is table infrastructure: it exists outside the fiction, but preserves the conditions that let the fiction remain playable. Section 25 covers this in full.

### 2. Materials

You need:

- two six-sided dice
- paper or a shared document
- one referee and one or more players
- or, for solo/low-referee play, the oracle rules in section 24

### 3. The basic cycle of play

Play proceeds in a simple loop.

1. **The referee describes the situation.**
2. **A player says what their character does.**
3. **The table decides whether the outcome is obvious, impossible, or uncertain.**
4. **If obvious, it happens.**
5. **If impossible, the referee says why and what would need to change.**
6. **If uncertain and consequential, roll.**
7. **The result creates a new fictional state.**
8. **Play continues from that changed situation.**

Do not roll to see whether play can continue. Roll only when the result will change the situation.

### 4. Characters

A character sheet contains only fictional facts.

Each player creates a character by answering these prompts.

#### Name

Who are you called?

#### Place in the world

What are you, socially or practically?

Examples:

- village surgeon
- disgraced knight
- smuggler-priest
- apprentice weather witch
- widowed cartographer
- deserter from the imperial army

#### Three truths

Write three things that are true about your character.

At least one should be a **capability**. At least one should be a **connection**. At least one should be a **vulnerability, burden, or complication**.

Examples:

- I can read old military fortifications.
- The dockworkers of Greyharbor still trust me.
- I panic when trapped underground.
- I carry my brother’s sword, though I never learned to use it properly.
- I know the songs that calm the dead.
- The magistrate believes I murdered her son.

These are not bonuses. They are facts. They determine what your character can do, what is easy, what is dangerous, who responds to them, and what consequences make sense.

#### Visible condition

Begin with one visible condition that tells the table how the character currently appears.

Examples:

- road-worn and hungry
- dressed above their station
- cheerful despite recent grief
- limping from an old injury
- alert, clean, and prepared

This condition may change during play.

#### Belongings

List what the character is carrying that matters.

Do not list every mundane object unless it is important. A character plausibly equipped for their role may have ordinary tools of that role unless the fiction says otherwise.

Examples:

- a surgeon’s satchel
- a patched traveling cloak
- a rusted pistol with one shot prepared
- three letters of introduction
- a mule named Candle
- lockpicks hidden in a boot heel

### 5. The character sheet

A character sheet looks like this:

``` text
Name:
Place in the world:

Truths:
1.
2.
3.

Visible condition:

Belongings:

Current states:
-
-
-
```

**Current states** are fictional conditions created during play.

Examples:

- bleeding from the ribs
- trusted by the miller’s family
- carrying a stolen temple key
- soaked, freezing, and shivering
- marked by the ash-spirit
- wanted in Eastgate
- separated from the others
- holding the bridge alone

A state stays on the sheet only while it is true inside the fiction.

Remove a state when it no longer changes what the character can do, what someone wants from them, or what danger they face. Do not let the sheet become an archive. A long list of states that no longer shape choices is dead overhead in a different form.

### 6. When to roll

Roll only when all three are true:

1. A character is attempting something specific.
2. The outcome is uncertain.
3. The outcome matters.

Do **not** roll for routine competence. Do **not** roll for actions whose consequences are uninteresting. Do **not** roll when the fiction already decides the answer.

A former locksmith opening a cheap lock in a quiet room opens it. A former locksmith opening an ancient lock while guards climb the stairs rolls. A farmer with no tools trying to open the same ancient lock may need to find another approach first.

The player always states both:

> **Intent:** what they want to accomplish. **Method:** how they try to do it.

“I get past the guard” is incomplete. “I flatter the guard by pretending to recognize his military service” is playable.

### 7. Before the roll

Before rolling, the referee should make the stakes clear enough that the player understands the risk.

Use plain fictional language.

Good:

> “You can try to cross the rotten beam, but if it gives way you may fall into the flooded chamber below.”

Good:

> “You can threaten him, but if he does not break, he will know you are desperate.”

Bad:

> “Failing this will give you a penalty later.”

Bad:

> “This will fill two boxes on the complication track.”

The player may change their approach after hearing the risk.

### 8. The roll

When a roll is needed, roll **2d6**.

Read the result:

#### 10 or more: **Yes, and**

The character’s intent succeeds, and the situation improves, opens, or clarifies.

Examples:

- The door opens silently, and you hear the guards before they reach the hall.
- The crowd believes you, and someone steps forward with useful information.
- You drive the beast back, and it exposes the chain around its throat.
- You cross the ravine, and the rope remains secure for the others.

#### 7–9: **Yes, but / No, but**

The situation changes, but imperfectly.

Usually, the character succeeds with a cost, delay, exposure, hard choice, or partial effect.

Sometimes, especially when the opposition is strong or the method is weak, the character does not get what they wanted, but they gain something useful, avoid the worst, or reveal a new way forward.

Examples:

- The door opens, but the hinges shriek.
- The guard lets you pass, but only if you leave your weapon with him.
- You wound the beast, but it pins you beneath its weight.
- You do not convince the magistrate, but her clerk secretly believes you.
- You leap the gap, but your pack tears loose and falls.

#### 6 or less: **No, and**

The character’s intent fails, or succeeds too late, and the situation worsens.

This does not stop play. It creates a new fictional state.

Examples:

- The lock breaks, and someone begins opening the door from the other side.
- The guard calls for help, and now the courtyard is waking.
- The beast knocks you down and closes its jaws around your arm.
- The magistrate names you a liar in front of the court.
- The beam snaps; you fall into the black water below.

Every result must answer:

> **What is now true in the fiction?**

Also ask:

> Who in the scene can perceive or respond to it? What is now open, closed, or urgent?

### 9. How character truths affect play

Character truths never add numbers to the roll.

They affect the fiction directly.

A relevant truth can do any of the following:

#### Make an action possible

A character who knows the songs that calm the dead can attempt to quiet a restless ghost. Another character may not be able to try without first learning, bargaining, or using some other method.

#### Make an action automatic

A trained sailor ties a reliable knot in ordinary conditions. No roll.

#### Change the stakes

A battlefield surgeon treating a wound under pressure still rolls, but failure may mean “the patient survives with a fever” rather than “the patient dies immediately.”

#### Change the available consequences

If a character is known and loved in a village, failure in a social confrontation may create disappointment, debt, or shame rather than immediate violence.

#### Invite trouble

A vulnerability is also real. If a character panics underground, then being trapped in a mine changes what they can plausibly do until someone steadies them or they regain control.

The rule is simple:

> If a trait is true, let it be true before touching the dice.

#### When the table disagrees

Because Thresholds relies on fictional judgment rather than numerical lookup, the table will sometimes disagree about whether a truth applies, whether an action is possible, or what consequence is fair. Handle disagreement without negotiation loops.

When the table is uncertain, ask in order:

1. **What is true inside the fiction right now?** Start from the established facts, not from what anyone wants to happen.
2. **Does the character's truth change what is plausible here?** If yes, in which direction: does it open, narrow, or change the stakes?
3. **What would the situation produce if no one were arguing?** Let the fiction answer before anyone argues a position.
4. **If it is still unclear, the referee decides.** State the decision plainly, name the fictional fact it rests on, and move on. Do not revisit it in the same scene.

The goal is a fictional fact in under thirty seconds, not a resolved debate. If a disagreement cannot be settled quickly, default to the option that keeps the fiction moving and revisit the underlying question between sessions.

### 10. Advantage and disadvantage

Do not use numerical modifiers.

Instead, describe what is actually true.

If the character has a clear advantage, the referee should usually do one of these:

- allow success without a roll
- reduce the danger of failure
- make success more complete
- offer more information before the choice
- let the character attempt something otherwise impossible

If the character is clearly hindered, the referee should usually do one of these:

- require a different approach first
- make the possible result narrower
- make the danger sharper
- say the action is impossible until the fiction changes

Examples:

- High ground is not “+1.” It means enemies must climb, arrows have clearer lines, and retreat may be easier.
- Darkness is not “disadvantage.” It means you cannot see the pit, faces are hard to read, and anyone silent may vanish.
- A fine sword is not a damage bonus. It cuts mail poorly, bites flesh cleanly, impresses duelists, and may be recognized.

### Where difficulty lives

Difficulty does not change the number on the dice. It changes the fiction before the roll: whether the action is possible, what preparation is needed, what success can accomplish, and what failure will cost.

A locked door in an empty corridor is different from the same lock while guards climb the stairs. The dice are the same. What the result means—and what it costs—is not.

When fictional advantage or difficulty changes what is possible or what success can accomplish, say so before the roll.

Example:

> "Because you have the high ground, failure will not mean they overwhelm you immediately; it will mean they force you back toward the broken stairs."

Since the dice do not change, the fictional effect of positioning, preparation, and expertise must be stated plainly before anyone picks up the dice. Otherwise players will feel that their choices made no difference.

### 11. Harm

There are no hit points.

When someone is harmed, say what happened to them in the fiction.

Examples:

- cut across the forehead, blood in the eyes
- winded and unable to speak
- left hand crushed
- arrow through the thigh
- terrified and backing away
- coughing black smoke
- concussed and confused
- dying unless treated now

A harm state matters because it changes what the character can do.

A character with an arrow through the thigh cannot sprint. A character coughing black smoke cannot hide quietly. A character terrified of the thing in the cellar may freeze, flee, bargain, or need help before facing it.

#### Severe harm

When a consequence could kill, maim, break, or permanently change a character, the referee should make that risk visible before the roll unless it is already obvious.

Good:

> “If you hold the door alone, you can buy them time, but the ogre may crush you against the frame.”

Good:

> “You can drink from the silver pool, but if the stories are true, it may change what you are.”

Death is allowed when it follows honestly from the fiction, but it should not arrive as a hidden mechanical surprise.

#### Recovery

Harm changes when the fiction changes.

A wound may need pressure, stitching, rest, medicine, magic, shelter, or time. A fear may need reassurance, distance, victory, confession, or ritual. A broken reputation may need restitution, proof, sacrifice, or public witness.

Erase or alter a harm state only when something inside the fiction has addressed it.

### 12. Conflict

There is no separate combat system.

A fight is just a dangerous situation where people are trying to change the fiction by force.

Do not roll for every swing. Roll when an action could meaningfully alter the situation.

Examples of meaningful combat intents:

- I drive him away from the doorway.
- I get inside his spear reach.
- I hold the stairs until the others escape.
- I disarm her without killing her.
- I put an arrow in the lantern above them.
- I tackle the assassin before he reaches the prince.
- I kill the wolf before it drags the child into the trees.

On a hit, create a new state. On a partial, create a new state with cost or complication. On a miss, create a new state that worsens the situation.

Possible combat states:

- disarmed
- cornered
- bleeding heavily
- separated from allies
- shield split
- weapon lodged in bone
- pinned beneath a body
- enemy morale breaking
- reinforcements arriving
- fire spreading through the room
- hostage exposed
- dead

The referee should treat weapons according to what they are.

A knife in a crowded room is different from a spear in a doorway. A pistol is different before and after it has fired. A warhammer against plate armor is different from a rapier in a duel.

Do not translate weapons into damage numbers. Let them change what is plausible.

### 13. Groups, help, and opposition

#### Helping

A character can help if they describe how their action changes the situation.

Good help:

- “I hold the lantern low so she can see the tripwire.”
- “I keep talking so the guard looks at me instead of him.”
- “I brace the door with my shoulder while she lifts the bar.”

Helping does not add a bonus. It changes what is possible, what is risky, or what success looks like.

If helping exposes the helper to danger, say so before the roll.

#### Group action

When several characters act together toward one intent, make one roll for the shared attempt.

Before rolling, the fiction should be clear enough to answer: who is most at risk, what each person is actually doing, and what will change if it goes wrong. If it is not clear, describe it first.

The result affects the group according to the fiction.

#### Opposition

NPCs do not roll against players by default.

When an NPC acts, the referee describes what they do and what danger or opportunity it creates. If a player resists, interrupts, exploits, or endures that action, the player may roll.

Example:

> Referee: “The captain orders two soldiers to bar the gate while she draws her pistol. If no one stops them, the gate will be sealed.”

A player might then say:

> “I rush the soldiers before the bar drops.”

Now there is a roll.

### 14. NPCs and creatures

NPCs and creatures are written as fictional profiles, not stat blocks.

Use this format:

``` text
Name:
What they want:
What they can do:
What shows on the surface:
What they know:
What they fear or need:
What would change them:
```

Example:

``` text
Name: Captain Ilyra Voss
What she wants: Keep the refugees outside the city until dawn.
What she can do: Command frightened soldiers; read military deception; shoot calmly at close range.
What shows on the surface: Controlled voice, sleepless eyes, hand always near pistol.
What she knows: The plague is already inside the city.
What she fears or needs: Proof that opening the gate will not start a riot.
What would change her: Public support from the temple, evidence of a worse threat outside, or someone taking responsibility before witnesses.
```

A creature may be written the same way.

``` text
Name: The Bell-Stag
What it wants: To drive intruders away from the ruined chapel.
What it can do: Move silently through fog; gore a horse; ring the iron bells grown into its antlers.
What shows on the surface: Wet fur, blind white eyes, bells that sound underwater.
What it knows: The chapel crypt is open.
What it fears or needs: Fire blessed with a funeral prayer.
What would change it: Returning the stolen bell-clapper to the grave below.
```

If an NPC is injured, persuaded, frightened, trapped, exposed, or killed, write that as a fictional state.

### 15. Equipment and resources

Track objects, not abstract inventory points.

A thing matters if its presence or absence changes what can happen.

Examples:

- rope
- lantern oil
- court clothing
- a forged seal
- a loaded pistol
- a winter cloak
- a sacred relic
- the last clean bandage

For ordinary belongings, use common sense. A traveling healer probably has bandages. A naked prisoner does not.

For scarce things, track the actual fiction.

Good:

- one torch left
- pistol unloaded
- waterskin empty
- mule carrying the heavy chest
- rope cut and hanging from the tower

Avoid:

- Supply 3
- Ammo 2
- Gear points
- Wealth rating

Money may be recorded when the exact amount matters inside the fiction. Otherwise, describe economic position.

Examples:

- broke and owing rent
- comfortable but watched by creditors
- rich in land, poor in coin
- carrying a purse heavy enough to attract thieves

### 16. Magic, powers, and extraordinary abilities

Magic is treated as part of the fiction, not as a spell list unless the setting demands one.

A magical ability should answer:

``` text
What can it change?
What does it require?
What does it look, sound, or feel like?
What can go wrong?
What does it cost inside the fiction?
```

Example:

``` text
Ash-speech
What it can change: Lets the caster speak with the recent dead through their ashes.
What it requires: A named corpse, a flame, and a question spoken aloud.
What it looks like: The ash rises into the shape of a mouth.
What can go wrong: The wrong dead answer, the corpse remembers pain, or something nearby learns the caster’s name.
What it costs: The caster’s voice becomes smoke-rough until they sleep.
```

Do not count spell slots unless the count itself is fictional.

Acceptable:

- three blessed matches
- one oath remaining before the saint turns away
- the moon will not answer again tonight
- the demon demands a secret each time

Avoid:

- 4 mana
- 2 spell points
- once per scene
- recharge on a short rest

A magical result should always create a fictional state.

### 17. Social play

Social actions are resolved the same way as any other action.

The player must say what their character says, offers, hides, threatens, proves, or reveals.

Do not roll to “be persuasive” in the abstract. Roll when a specific approach meets uncertain resistance.

NPCs are not puppets. A roll cannot make someone do something that the fiction gives them no reason to do. It can create pressure, trust, fear, obligation, confusion, temptation, shame, or opportunity.

Examples:

- The duke will not abandon his city because of one speech.
- The duke might delay the execution if presented with evidence.
- The guard will not betray her sister for a bribe.
- The guard might look away if convinced the prisoner is already doomed.
- The mob will not become calm because someone rolled well.
- The mob might split, hesitate, or turn its anger toward a new target.

On a partial success, social consequences should remain social and fictional.

Examples:

- They agree, but want a witness.
- They believe you, but ask how you know.
- They let you pass, but remember your face.
- They keep your secret, but now you owe them.
- They are moved, but someone else in the room is offended.

### 18. Exploration and discovery

The referee describes what characters could notice from their position, senses, tools, and knowledge.

Do not hide essential information behind a failed roll. If the characters need a fact to make meaningful choices, give them the fact through the fiction.

Roll when the act of discovery is uncertain and consequential.

Examples:

- searching the room before the guards return
- reading the old map while rain destroys the ink
- following tracks through a storm
- examining a corpse without attracting the family’s attention
- listening at a door while something listens back

A discovery roll should produce discovery, danger, or both.

10+:

> You find the hidden stair, and you know it was used recently.

7–9:

> You find the hidden stair, but opening it will scrape loudly unless you take time you may not have.

6 or less:

> You find the hidden stair because something below opens it first.

Failure should not mean “you learn nothing; try somewhere else” unless that itself changes the situation.

### 19. Time, travel, and pressure

Do not track time unless time matters.

When time matters, track it in fictional terms.

Examples:

- dawn is one hour away
- the tide is rising around the causeway
- the wedding begins when the third bell rings
- the fever will break or kill him before morning
- the soldiers will arrive after they search two more houses

A non-diegetic clock may be used only as table infrastructure, and only if each step corresponds to a visible or knowable fictional change.

Good clock:

``` text
The manor burns:
1. Smoke fills the upper hall.
2. The west stairs collapse.
3. The nursery catches.
4. The roof falls in.
```

Bad clock:

``` text
Danger: 0/6
```

Travel should focus on choices, discoveries, costs, and changed circumstances. Do not roll for every day of movement unless each result changes the fiction.

### 20. Referee principles

The referee should follow these principles.

#### Say what is true

Describe the situation honestly according to what is established, prepared, or implied.

#### Keep the fiction visible

Consequences should appear as things characters can perceive, suffer, use, or respond to.

#### Roll late

Let players gather information, make plans, use tools, and change the situation before dice appear.

#### Resolve once

One roll should produce a playable outcome. Do not require a second roll to find out what the first roll meant.

#### Make failure active

A failed roll should introduce action, pressure, revelation, loss, movement, or transformation.

#### Use the characters’ truths

A character’s background, relationships, fears, injuries, tools, and promises are part of the world. Bring them into consequences.

#### Do not protect plans

If the players’ choices would change the situation, let the situation change.

#### Do not punish curiosity

Make investigation dangerous, costly, revealing, or transformative. Do not make it empty.

#### Name consequences plainly

“The knife goes into your side” is better than “take harm.” “The priest now knows you lied” is better than “mark suspicion.”

### 21. Player principles

Players should follow these principles.

#### Act from the situation

Say what your character does, not which mechanic you want to trigger.

#### Treat consequences as real

A failed roll is not a pause or a mistake. It is the world changing.

#### Use what is true

Your belongings, wounds, relationships, memories, promises, and fears are tools and constraints.

#### Ask about the fiction

Useful questions include:

- What can I see from here?
- Does the guard seem frightened or loyal?
- How far away is the door?
- What would I need to cross safely?
- Who in this room looks uncertain?
- What does my old military training tell me?

#### Let the character change

If the fiction wounds, frightens, tempts, exposes, redeems, or transforms your character, write that down.

### 22. Advancement

Characters do not gain levels.

At the end of a session, each player may change the sheet if the fiction justifies it.

Use these prompts:

- What did you learn to do?
- Who now trusts, fears, hates, or needs you?
- What wound, fear, debt, or promise follows you?
- What belief changed?
- What object, title, secret, or responsibility did you gain?
- What part of your old life is no longer true?

A character may add, remove, or rewrite one truth when play has made the change undeniable. The "one truth" limit is a default to keep the sheet legible between sessions, not a claim that the fiction only changed one thing. If play has undeniably changed more, record the overflow in the campaign record and return to it at the next session.

Examples:

- “I served the crown faithfully” becomes “I deserted when I learned what the crown buried.”
- “The dockworkers trust me” becomes “The dockworkers think I sold them out.”
- “I cannot bear enclosed spaces” becomes “I can enter the dark if Mara is with me.”
- “I know only village medicine” becomes “I have learned the plague-signs of the old empire.”

Advancement is not increased power. It is changed fictional position.

### 23. Campaign record

Keep a shared record of facts that must remain stable between sessions.

Use headings like:

``` text
People:
Places:
Promises:
Threats:
Open questions:
Changed states:
```

Examples:

``` text
People:
- Captain Voss opened the gate after Teren took public responsibility.
- The bell-ringer child is still missing.

Places:
- The west road is flooded.
- The chapel crypt is open.

Promises:
- Mara owes the ash-spirit one secret.
- The refugees promised to speak for the characters at trial.

Threats:
- The city plague is already inside the walls.
- Something wearing Father Caldus’s face was seen near the well.

Open questions:
- Who stole the bell-clapper?
- Why does the plague spare children?
```

The campaign record is not a score sheet. It preserves continuity so the fiction can remain coherent.

Prune it between sessions. Keep only entries that constrain current choices, create current danger, or open current questions. An entry that describes something fully resolved and without ongoing consequence has become dead overhead. Remove it or move it to a separate archive if you want to preserve it as history.

### 24. Optional oracle for solo or low-referee play

When no referee is available, or when the referee wants to disclaim an answer, ask a closed question about the fiction.

Examples:

- Is the door guarded?
- Does she recognize me?
- Has the bridge already collapsed?
- Is the ghost angry?
- Did the letter arrive first?

Roll 2d6.

#### Oracle answer

|   Roll | Answer                                                        |
|-------:|---------------------------------------------------------------|
|    10+ | Yes, and something else is also true.                         |
|   7–9  | Yes, but there is a complication or cost. / No, but something useful, revealing, or dangerous is present. |
| 6 or less | No, and the situation worsens.                             |

The oracle must answer as a fictional event or fact, not as a prompt that requires another procedure.

After rolling, complete the answer in one concrete sentence before doing anything else. The sentence must name something the characters could observe, discover, suffer, use, or be changed by.

Good:

> “Yes, the door is guarded, and the guard is arguing with someone you know.”

Good:

> “No, the bridge has not collapsed, but the supports are groaning under floodwater.”

Bad:

> “Introduce a complication.”

Bad:

> “Generate a theme.”

If the result is unclear, choose a direction from this list and complete it as a sentence:

- reveal a person's action or location
- change the environment
- expose a danger
- consume or damage an object
- alter someone's attitude toward a character
- advance a visible threat
- reveal a hidden fact
- separate, trap, mark, wound, or obligate someone

### 25. Safety and table agreement

Before play, agree on material that should be excluded, softened, or handled off-screen.

This is table infrastructure, not part of the fiction, but it supports the conditions for play.

Use whatever safety tools the table prefers. Keep them simple and available.

Also agree on the game’s orientation:

> We are here to treat the fiction as the shared object of play. We will make choices from inside the situation. We will let consequences change the world and the characters. We will not treat the rules as a tactical puzzle or a way to win.

### 26. Running the first session

To begin quickly, the referee prepares a charged situation, not a plot.

Prepare:

``` text
A place:
A present danger:
Three people who want different things:
One thing hidden from the characters:
One thing already going wrong:
Three sensory details:
A question you do not know the answer to:
```

Example:

``` text
A place:
The river gate of a walled city at midnight.

A present danger:
Refugees outside the gate may carry plague; something worse follows them.

Three people who want different things:
- Captain Voss wants the gate closed until dawn.
- Sister Elian wants the sick brought inside.
- A masked merchant wants one particular child kept outside.

One thing hidden from the characters:
The plague is already inside the city.

One thing already going wrong:
The river is rising against the lower gate.

Three sensory details:
- Wet rope creaking in the dark.
- Lanterns reflected in floodwater.
- A bell ringing once, then stopping.

A question you do not know the answer to:
Who will the characters choose to save first?
```

Start with the characters present and under pressure.

Do not decide how the situation ends.

### License

Thresholds by Roberto Bisceglie is marked CC0 1.0 Universal. To view a copy of this mark, visit https://creativecommons.org/publicdomain/zero/1.0/
